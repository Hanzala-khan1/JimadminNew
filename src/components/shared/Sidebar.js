...
const Sidebar = ({ handleShowNav }) => {
    ...
    if (!user.isAdmin && !user.isJimAdmin) {
        user.BusinessLocation.forEach(Gym => {
            JimsList.push({ label: `${Gym.Gym.name}`, icon: faCircle, itemId: Gym.Gym._id.toString(), is_Active: (activegym == Gym.Gym._id.toString() ? true : false) })
        });
    }
    ...
    const menuItems = {
        ...
        user: [
            {
                label: 'Switch Gym', submenu: JimsList, icon: faDumbbell
            },
            { label: 'Dashboard', link: '/', icon: faHome },
            { label: 'Packages', link: '/newpackages', icon: faCubes },
    ...
    console.log("isActiveGymsOpenisActiveGymsOpenisActiveGymsOpen", isActiveGymsOpen)
                        ...
                        <ul className="">
                            {item.submenu.map((subItem, subIndex) => {
                                ...
                                return (<>
                                    ...
                                    {!subItem.link ? (
                                        <li className={`menu-item ${subItem.is_Active ? "active" : ""}`} key={subIndex}>
                                            <div onClick={() => handleActiveGym(subItem.itemId)} className="menu-link "> {subItem.label}</div>
                                        </li>
                                    ) : (
                                        <>
                                            <li className={`menu-item ${isSubActive ? "active" : ""}`} key={subIndex}>
                                                <Link to={subItem.link} className="menu-link">       <div>{subItem.label}</div></Link>
                                            </li>
                                        </>
                                    )}
                                </>
    ...
    return (
        <aside id="layout-menu" className={`layout-menu menu-vertical menu bg-menu-theme pt-4 show layout-menu-expanded`} >
            <div className="app-brand demo mb-4">
                ...
                <a href="javascript:void(0);" className="layout-menu-toggle menu-link text-large ms-auto">
                    ...
                    <i className="ti ti-x d-block d-xl-none ti-sm align-middle" onClick={handleShowNav}></i>
                </a>
            </div>
